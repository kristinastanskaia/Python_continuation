...
logging.basicConfig(level=logging.INFO, filename="logs_date_validate.log",
                    filemode="a",
...
def parcer():
    ...
    устанавливается значение текущего дня,месяца
...
def is_valid_date(date, month, year) -> None:
    """
    ...
    if year < 1 or year > 9999 or month < 1 or month > 12 or date < 1 or\
            date > 31:
        return logging.warning("Невалидная дата")
    if month in (4, 6, 9, 11) and date > 30:
        return logging.warning("Невалидный день, в апреле, июне, сентябре "
                               "и ноябре не может быть более 30 дней")
    if month == 2 and is_leap(year) and date > 29:
        return logging.warning("Невалидный день, в  феврале високосного года "
                               "не может быть более 29 дней ")
    if month == 2 and not is_leap(year) and date > 28:
        return logging.warning("Невалидный день, в  феврале не високосного не "
                               "может быть более 28 дней ")
