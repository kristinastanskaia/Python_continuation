# На вход программе подаются два списка, каждый из которых содержит 10
...
class LotteryGame:
    ...
    def compare_lists(self):
        """
        Функция для проверки совпадения чисел в выпавших числах и числах
        пользователя
        """
        result_list = []
...
def parser():
    ...
    parcer.add_argument("-u", "--userlist",
...
logging.basicConfig(level=logging.INFO, filename="lottery_logs.log",
                    filemode="a",
...
try:
    ...
    if userlist_len == droppedlist_len:
        ...
        logging.error(
            f"Введено невалидное количество чисел {userlist_len} вместо"
            f" {droppedlist_len}")
